...
const convertToCmyk = hex => {
  ...
  // converts array items to string with cmyk letter
  const stringCmykArr = convertedHex.map((i, index) => {
    const stringNum = `${i.toString()}`;
    let letterNum;
    if (index === 0) {
      letterNum = `c${stringNum}`;
    } else if (index === 1) {
      letterNum = `m${stringNum}`;
    } else if (index === 2) {
      letterNum = `y${stringNum}`;
    } else {
      letterNum = `k${stringNum}`;
    }
    return letterNum;
  });

  // converts cmyk array to one string
  const cmykString = stringCmykArr.join(' ');
  return cmykString;
};
