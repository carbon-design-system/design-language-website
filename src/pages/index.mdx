import HomepageTemplate from 'gatsby-theme-carbon/src/templates/Homepage';
import HomepageTile from '../components/HomepageTile';
import Carousel from '../components/Carousel';

export default HomepageTemplate;

<Row classname='homepage-row'>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      hoverDark
      transparentImage="images/tooltip-components.svg"
      subtitle="Components"
      title="Carbon X"
      actionIcon="resources"
      link="https://www.carbondesignsystem.com/"
      target="_blank"
    />
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="Design Kit"
      title="Explore & Create"
      actionIcon="resources"
      identityIcon="images/SketchIcon.svg"
      link="https://www.carbondesignsystem.com/getting-started/designers#get-the-kit"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="Elements"
      title="2x Grid"
      actionIcon="article"
      transparentImage="images/grid-temp.png"
      link="https://www.ibm.com/design/language/elements/2x-grid/"
      target="_blank"
    />
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="GitHub Repo"
      title="Design & development"
      actionIcon="resources"
      identityIcon="images/GitHubIcon.svg"
      link="https://github.com/IBM/carbon-elements"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:2"
      theme="dark"
      hoverDark
      backgroundColor="#8a3ffc" // if omitted, defaults to #282828
      transparentImage="images/three-icons-stacked.svg"
      subtitle="Iconography"
      title="UI icons"
      actionIcon="resources"
      link="https://www.ibm.com/design/language/elements/icons/design"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="2:1"
      theme="dark"
      subtitle="Elements"
      title="Color in UI"
      actionIcon=""
      link="https://www.carbondesignsystem.com/guidelines/color/overview"
      target="_blank"
    />
    <HomepageTile
      ratio="1:1"
      marginTop="50%"
      theme="dark"
      hoverDark
      contentOnHover
      image="images/blue1.jpg"
      subtitle="Elements"
      title="Color Palette"
      actionIcon="article"
      link="https://www.ibm.com/design/language/elements/color/"
      target="_blank"
    />
  </Column>
  <Column colLg={8} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      hoverDark
      image="images/landing-color-grid.svg"
      subtitle="Elements"
      title="Color Palette"
      actionIcon="article"
      link="https://www.ibm.com/design/language/elements/color/"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      backgroundColor="rgb(6, 31, 128)" // if omitted, defaults to #282828
      transparentImage="images/type-scales.svg"
      actionIcon="no-icon"
      link="https://www.ibm.com/design/language/elements/type-specs-ui/"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="Elements"
      title="Type Specs"
      actionIcon="article"
      link="https://www.ibm.com/design/language/elements/type-specs-ui/"
      target="_blank"
    />
  </Column>
</Row>
<Row classname='homepage-row'>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      image="images/landing-plex-animation.gif"
      actionIcon="no-icon"
      link="https://www.ibm.com/design/language/elements/type-basics/"
      target="_blank"
    />
    <HomepageTile
      ratio="2:1"
      theme="dark"
      subtitle="Elements"
      title="Typeface"
      actionIcon="article"
      link="https://www.ibm.com/design/language/elements/typeface/"
      target="_blank"
    />
  </Column>
  <Column colLg={8} noGutterSm>
  <HomepageTile
    ratio="1:1"
    image="images/product-gallery.jpg"
    actionIcon="no-icon">
    <HomepageTile
      ratio={{ default: '2:1', md: '4:1', }}
      theme="dark"
      subtitle="Gallery"
      title="IBM Services"
      actionIcon="article"
      link="https://www.ibm.com/design/language/gallery/"
      target="_blank"
    />
    </HomepageTile>
  </Column>
</Row>
<Row classname='homepage-row'>
  <Column colLg={12} noGutterSm>
    <HomepageTile
      ratio={{ default: '1:2', md: '16:9' }}
      actionIcon="no-icon">
      <Carousel id="c1" count="1 2" nav={false} fade={true} autoPlay>
        <img
          src="images/landing-photography-gallery-1-mobile.jpg"
          alt="alt text"
        />
        <img
          src="images/landing-photography-gallery-2.jpg"
          alt="alt text"
        />
      </Carousel>
      <HomepageTile
        ratio={{ default: '2:1', md: '4:1', lg: '6:1' }}
        theme="dark"
        subtitle="Elements"
        title="Photography"
        actionIcon="article"
        link="https://www.ibm.com/design/language/elements/photography/"
        target="_blank"
      />
    </HomepageTile>
  </Column>
</Row>
<Row className="homepage-row homepage-tiles--end">
  <Column colLg={8} noGutterSm>
    <HomepageTile
      ratio={{ default: '1:2', md: '1:1' }}
      image="images/landing-gallery-watson-duo-phone.jpg"
      actionIcon="no-icon"
    >
      <HomepageTile
        ratio={{ default: '2:1', md: '4:1' }}
        theme="dark"
        subtitle="Case Study"
        title="Coming soon"
        actionIcon="disabled"
        disabled
      />
    </HomepageTile>
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="Philosophy"
      title="Principles"
      actionIcon="article"
      link="https://www.ibm.com/design/language/philosophy/principles"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      hoverDark
      backgroundColor="#bb8eff"
      image="images/landing-illustration-ibm-rand-logo.svg"
      actionIcon="no-icon"
      link="https://www.ibm.com/design/language/philosophy/"
      target="_blank"
    />
  </Column>
  <Column colLg={4} colMd={4} noGutterSm>
    <HomepageTile
      ratio="1:1"
      theme="dark"
      subtitle="Philosophy"
      title="Start smart"
      transparentImage="images/big-arrow.svg"
      actionIcon="no-icon"
      link="https://www.ibm.com/design/language/philosophy/"
      target="_blank"
    />
  </Column>
</Row>
