---
title: UI icons
description: Icons are important aids to the user experience. Employ icons strategically with careful view to sizing, alignment, clearance and color in usage.
tabs: [Library, Design, Usage, Contribute]
---

<PageDescription>

Iconography is highly functional in a user interface. When used wisely, icons become an elegant yet efficient way to communicate with and help guide a user through an experience. To maintain this functionality, it's important to reduce cognitive load on users by employing icons sparingly and strategically throughout your designs.
</PageDescription>

<AnchorLinks>
  <AnchorLink>Sizing</AnchorLink>
  <AnchorLink>Alignment</AnchorLink>
  <AnchorLink>Clearance</AnchorLink>
  <AnchorLink>Color</AnchorLink>
</AnchorLinks>

<Video src="/videos/icon-usage.mp4" poster="/images/icon-usage-poster.jpg" />

## Sizing

Icons have been designed to work best in four sizes: 16px, 20px, 24px, and 32px. Please use icons at their originally produced size. These are provided in SVG and Adobe Illustrator files.

<ArtDirection>

![sizing example](../images/sizing-example-comp-mobile.svg)
![sizing example](../images/sizing-example-comp.svg)

</ArtDirection>

| Icon size                                     | Stroke width | Padding | Live area | Corner radius | Type sizes |
| --------------------------------------------- | ------------ | ------- | --------- | ------------- | ---------- |
| 32px                                          | 2px          | 2px     | 28px      | 2px           | —          |
| 24px                                          | 1.5px        | 1.5px   | 21px      | 1.5px         | —          |
| <span class="footnote__highlight">20px</span> | 1.25px       | 1.25px  | 17.5px    | 1.25px        | 16pt       |
| <span class="footnote__highlight">16px</span> | 1px          | 1px     | 14px      | 1px           | 14pt       |

<Caption>
  <span class="footnote__highlight" />
  <span class="footnote">
    16px and 20px icons are optimized to feel balanced when paired with 14pt and
    16pt IBM Plex. Use 24px and 32px when larger icons are needed.
  </span>
</Caption>
<ArtDirection>

![](../images/sizing-alignment-example-comp-mobile.svg)
![](../images/sizing-alignment-example-comp.svg)

</ArtDirection>

<DoDontRow>
<DoDont caption="Do use the correct icon size with IBM Plex.">

![correct icon sizing](../images/sizing-correct-size-do.svg)

</DoDont>
<DoDont type="dont" caption="Do not alter icon sizes.">

![incorrect icon sizing](../images/sizing-correct-size-dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont caption="Do center-align icons when they’re next to text.">

![correct sizing alignment](../images/sizing-alignment-do.svg)

</DoDont>
<DoDont type="dont" caption="Do not baseline-align icons to the text.">

![incorrect sizing alignment](../images/sizing-alignment-dont.svg)

</DoDont>
</DoDontRow>

## Alignment

Icons are optically aligned to the center of the icon grid within the boundary box. Centering ensures all icons will be aligned correctly when exported and used side-by-side in a line or a row. Visually center an icon where the visual weight is heaviest. In some situations, mechanically centering doesn’t work.

![alignment example](../images/alignment-example-comp.svg)

<ArtDirection>

![](../images/alignment-example-comp-visual-center-mobile.svg)
![](../images/alignment-example-visual-center-comp.svg)

</ArtDirection>

## Clearance

When designing icons for touch surfaces, all targets need to be 44px or greater. The icon can be smaller than 44px if padding is added to the touch target with CSS to meet the 44px minimum.

<ArtDirection>

![clearance exmaple](../images/clearance-example-comp-mobile.svg)
![clearance exmaple](../images/clearance-example-comp.svg)

</ArtDirection>

<DoDontRow>
<DoDont caption="Do follow the clearance rule to allow for legibility and an optimal touch experience.">

![clearance rule](../images/clearance-do.svg)

</DoDont>
<DoDont type="dont" caption="Do not collapse the icon clearance area.">

![incorrect clearance](../images/clearance-dont.svg)

</DoDont>
</DoDontRow>

## Color

Icons are always a solid, monochromatic color and need to pass the same color contrast ratio as typography (4.5:1). The color of the icon should reflect the importance of the icon’s action which should always be to help guide a user. For more information on color, see Color in UI.

<DoDontRow>
<DoDont caption="Do match your icon color with your text color when pairing them.">

![color match](../images/color-match-do.svg)

</DoDont>
<DoDont type="dont" caption="Do not use different colors for text and icons.">

![incorrect color match](../images/color-match-dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont caption="Do make icons monotone.">

![color icon](../images/colored-icon-do.svg)

</DoDont>
<DoDont type="dont" caption="Do not use more than one color within an icon.">

![incorrect color icon](../images/colored-icon-dont.svg)

</DoDont>
</DoDontRow>
