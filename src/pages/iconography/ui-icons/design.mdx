---
title: UI icons
description:
  The style, strokes, perspective, corners and angles of IBM icons all echo our
  Plex(R) typeface. Use them to represent ideas, invite action or draw
  attention.
tabs: [Library, Design, Usage, Contribute]
---

<PageDescription>

As visual symbols, IBM user interface (UI) icons represent ideas, objects or actions. They can communicate messages at a glance, afford interactivity, and draw attention to important information. Based on details from the IBM Plex® typeface, they work well at small sizes.

</PageDescription>

<AnchorLinks>
  <AnchorLink>Foundation</AnchorLink>
  <AnchorLink>Style</AnchorLink>
  <AnchorLink>Padding</AnchorLink>
  <AnchorLink>Perspective</AnchorLink>
  <AnchorLink>Corners</AnchorLink>
  <AnchorLink>Angles</AnchorLink>
</AnchorLinks>

## Resources

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="UI icons: Starter file"
  actionIcon="download"
  href="https://github.com/carbon-design-system/carbon/raw/main/packages/icons/master/ui-icon-master.ai"
>

![Adobe Illustrator](../../../images/resource-cards/illustrator.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="UI icon: Figma library"
  href="https://www.figma.com/file/J5c0d85dSJn9JnBhSYYLmD/Icons---IBM-Design-Language?type=design&node-id=129%3A2&mode=design&t=IgykKAObMrV05Sic-1"
  actionIcon="launch"
>

![Figma Icon](../../../images/resource-cards/figma.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="UI icon: Carbon library"
  href="https://github.com/carbon-design-system/carbon/tree/master/packages/icons/src/svg"
>

![Carbon Icon](../../../images/resource-cards/carbon.png)

</ResourceCard>
</Column>
</Row>

## Foundation

The square grid is the underlying fabric of all IBM icons and is used as the foundation to determine line thickness, proportion, shape and positioning across the entire set of icons. To ensure a unified approach, the grid helps guide design decisions and allows flexibility in creating the appropriate shape needed to communicate the right idea.

<Video src="./assets/01-idl-iconography-ui-icons-design.mp4" />

### Base grid

IBM icons are drawn on a pixel-based grid of 32px x 32px and scaled down linearly to different sizes. Use the grid as your basic guideline to snap the artwork in place. We recommend fine-tuning adjustments for the shape you are trying to achieve during creation.

![](./assets/02-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont colMd={6} colLg={6} caption="Align design elements to the pixel grid.">

![](./assets/03-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont colMd={6} colLg={6} type="dont" caption="Avoid random decimal points in the x- and y- coordinates.">

![](./assets/04-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont colMd={6} colLg={6} caption="Do place arrow points on the grid.">

![](./assets/05-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont colMd={6} colLg={6} type="dont" caption="Avoid placing arrow points off the grid.">

![](./assets/06-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont colMd={6} colLg={6} caption="Do place at least one point of the angled stroke caps on the grid, if possible. ">

![](./assets/07-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont colMd={6} colLg={6} type="dont" caption="Avoid placing all angled stroke caps off the grid.">

![](./assets/08-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

### Padding

The grid contains 2px padding. This ensures icons will retain their desired scale and surrounding white space when exported. Only extend artwork into the padding for additional visual weight or for specific details required to define the shape’s content, meaning or character.

![](./assets/09-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont caption="Icon should remain inside the padding.">

![](./assets/10-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Avoid placing part of the icon in the padding area.">

![](./assets/11-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont caption="Do extend icons into padding if additional space is needed.">

![](./assets/12-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t crowd the design elements—make sure there is enough space between them.">

![](./assets/13-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

### Key shapes

Key lines give you consistent sizes for basic shapes or proportions across the icon set. This makes it easier to create a visually stable foundation and helps to establish relationships between the similarly proportioned icons and the objects or ideas they represent.

![](./assets/14-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont caption="Do use the keyshape that best demonstrates the proportion of the metaphor.">

![](./assets/15-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t use keyshapes that don’t reflect the real form of the metaphor.">

![](./assets/16-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont caption="Do extend content beyond the keyshapes for proper form if needed.">

![](./assets/17-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t force the content to fit inside the keyshape.">

![](./assets/18-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

## Style

The stylistic conventions of IBM icons create a meaningful bond with our typeface, [IBM Plex®](../../typography/typeface). Each icon shares distinct details and characteristics with the letterforms. The following video demonstrates some of these relationships.

<Video src="./assets/19-idl-iconography-ui-icons-design.mp4" />

<Row className="mock-gallery">
<Column colMd={6} colLg={6}>

![](./assets/20-idl-iconography-ui-icons-design.png)

<Caption>Rounded exteriors with 90° interiors</Caption>

</Column>
<Column colMd={6} colLg={6}>

![](./assets/21-idl-iconography-ui-icons-design.png)

<Caption>Distinctive point on tips</Caption>

</Column>
</Row>
<Row className="mock-gallery">
<Column colMd={6} colLg={6}>

![](./assets/22-idl-iconography-ui-icons-design.png)

<Caption>Slab characteristics</Caption>

</Column>
<Column colMd={6} colLg={6}>

![](./assets/23-idl-iconography-ui-icons-design.png)

<Caption>Square caps</Caption>

</Column>
</Row>

## Padding

The grid contains 2px padding. This ensures icons will retain their desired scale and surrounding white space when exported. Extend artwork into the padding only for additional visual weight or for specific details required to define the shape’s content, meaning or character.

![](./assets/24-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont caption="Do be consistent with 2px stroke weights.">

![](./assets/25-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t use inconsistent stroke weights. They’ll feel unbalanced and look like a mistake.">

![](./assets/26-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

## Perspective

When creating a new icon for the library, avoid dimensional or perspective-based representations. Use objective angles such as straight-on or profile views to ensure consistency.

<DoDontRow>
<DoDont caption="Use a straight-on angle for the icon.">

![](./assets/27-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t create an icon with dimension.">

![](./assets/28-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

## Corners

Use a consistent corner radius of 2px for rounded shapes. The 2px radius can be increased by a multiple of two when necessary to make the icon’s metaphor understandable or object shape clearly defined. Use an additional radius to make the metaphor reflect the real form of the object.

![](./assets/29-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont caption="Do use squared corners when needed to reflect the real form of the metaphor.">

![](./assets/30-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t force rounded corners if they don’t work for your metaphor.">

![](./assets/31-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont caption="Use square arrow tips.">

![](./assets/32-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t use rounded arrow tips.">

![](./assets/33-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>

## Angles

Use 45° angles for even anti-aliasing. Use increments of 15° whenever possible for other angles needed to best depict the shape you’re creating for your metaphor. You can create harmony across the icon set by consistently making angles sit on the same increments.

![](./assets/34-idl-iconography-ui-icons-design.png)

![](./assets/35-idl-iconography-ui-icons-design.png)

<DoDontRow>
<DoDont caption="Do use multiples of 15° or an angle that best represents the metaphor when necessary.">

![](./assets/36-idl-iconography-ui-icons-design.png)

</DoDont>
<DoDont type="dont" caption="Don’t use 45° angles exclusively for all icons. It won’t work.">

![](./assets/37-idl-iconography-ui-icons-design.png)

</DoDont>
</DoDontRow>
