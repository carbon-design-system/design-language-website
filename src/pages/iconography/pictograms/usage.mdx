---
title: Pictograms
description:
  Pictograms are visual symbols used to represent ideas, objects, or narratives
  at a glance. They work well in presentations and marketing communications.
tabs: [Library, Design, Usage, Contribute]
---

import { Bee } from '@carbon/icons-react';
import * as styles from './custom-table.module.scss'

<PageDescription>

Use pictograms to communicate at a glance, offer interactivity, or simplify complex ideas. 
This page shows examples for many use cases such as websites, product UIs, signage, events and merchandise. 
When used appropriately, they become an elegant addition to the IBM experience.

</PageDescription>

<AnchorLinks>
  <AnchorLink>Productive vs. expressive</AnchorLink>
  <AnchorLink>Sizing</AnchorLink>
  <AnchorLink>Alignment</AnchorLink>
  <AnchorLink>Containers</AnchorLink>
  <AnchorLink>Clearance</AnchorLink>
  <AnchorLink>Color</AnchorLink>
  <AnchorLink>Pictograms as illustrations</AnchorLink>
  <AnchorLink>Pictograms in action</AnchorLink>
</AnchorLinks>

## Productive vs. expressive

Productive and expressive pictograms are two distinct types of artwork that work best in different contexts. 
Productive pictograms are the go-to pictogram type for most contexts; they work across a variety of scales and environments, whether physical or digital. 
They’re illustrative and simple, allowing for one or many to be used to convey complex ideas.

Expressive pictograms represent a more dynamic option to the standard pictogram, using gradients, layering and transparency to build a sense of depth and movement in the artwork. 
Due to their visual complexity, expressive pictograms should be used selectively and only in scenarios that call for a graphic with strong presence.

<Tabs>
<Tab label="Productive">

<ArtDirection>

![Expressive pictogam example](images/usage_v2/01-idl-iconography-pictograms-usage.png)
![Expressive pictogam example](images/usage_v2/01-idl-iconography-pictograms-usage.png)

</ArtDirection>

<Row>
<Column colMd={8} colLg={12} noGutterSm>
  
<div className={styles.tableContainer}>
    <table className="page-table">
      <thead>
        <tr>
          <th className={styles.tableHeader}>Productive pictograms in context</th>
          <th className={styles.tableHeader}></th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>Use productive pictograms to deliver clear, consistent visual messaging across interfaces and environments. Their simplicity and adaptability make them ideal for most applications.</td>
          <td/>
        </tr>
      </tbody>
    </table>
</div>

</Column>
</Row>

</Tab>
<Tab label="Expressive">

<ArtDirection>

![Expressive pictogam example](images/usage_v2/02-idl-iconography-pictograms-usage.png)
![Expressive pictogam example](images/usage_v2/02-idl-iconography-pictograms-usage.png)

</ArtDirection>

</Tab>
</Tabs>

<DoDontRow>
<DoDont caption="Treat pictograms as illustations with sufficient sizing.">

![Pictogram with sufficient sizing](images/13_Usage_do.svg)

</DoDont>
<DoDont type="dont" caption="Don’t use pictograms as a replacement for UI icons. That is not their purpose.">

![Pictograms at UI icon scale](images/13_Usage_dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont caption="Use expressive pictograms as large, bold graphics.">

![Expressive pictogram as hero graphic](images/usage_v2/05-idl-iconography-pictograms-usage.png)

</DoDont>
<DoDont type="dont" caption="Don’t overuse expressive pictograms. They should be used sparingly.">

![Don’t overuse expressive pictograms](images/usage_v2/06-idl-iconography-pictograms-usage.png)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont type="dont" caption="Don’t use productive pictograms as logos or in a lockup for product headers, merchandise or events.">

![Don’t use pictograms as logos](images/usage_v2/07-idl-iconography-pictograms-usage.png)

</DoDont>
<DoDont type="dont" caption="Don’t use expressive pictograms as logos or in a lockup for product headers, merchandise or events.">

![Don’t use pictograms as logos](images/usage_v2/08-idl-iconography-pictograms-usage.png)

</DoDont>
</DoDontRow>

## Sizing

Pictograms are used in a range of sizes, the minimum being 48px. 
The maximum size may vary based on application. 
Use pictograms at their original sizes or scale at accepted increments.

<Tabs>
<Tab label="Productive">

<ArtDirection>

![Expressive pictogam example](images/14_Sizing_2-mobile.svg)
![Expressive pictogam example](images/usage_v2/09-idl-iconography-pictograms-usage.png)

</ArtDirection>

</Tab>
<Tab label="Expressive">

<ArtDirection>

![Expressive pictogam example](images/usage_v2/10-idl-iconography-pictograms-usage.png)
![Expressive pictogam example](images/usage_v2/10-idl-iconography-pictograms-usage.png)

</ArtDirection>

</Tab>
</Tabs>

## Alignment

Pictograms are optically aligned to the center of the icon grid within the boundary box. 
Centering ensures that all pictograms will be aligned correctly when exported and used side by side.

<ArtDirection>

![Alignment Example](images/15_Alignment_1-mobile.svg)
![Alignment Example](images/15_Alignment_1.svg)

</ArtDirection>

## Containers

Pictograms can be represented in a circular or rectangular container calculated based on the padding size.

<ArtDirection>

![Padding Example](images/16_Containers_1-mobile.svg)
![Padding Example](images/16_Containers_1.svg)

</ArtDirection>

<DoDontRow>
<DoDont caption="Keep pictograms at scale and optically centered in the container when necessary.">

![keep scale and optically center pictograms](images/16_Containers_3_do.svg)

</DoDont>
<DoDont type="dont" caption="Don’t resize pictograms outside of accepted proportions.">

![adhere to accepted proportions](images/16_Containers_4_dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont caption="Do use accepted shapes—circle or square—for containers.">

![use accepted container shapes](images/16_Containers_5_do.svg)

</DoDont>
<DoDont type="dont" caption="Don’t create new shapes for containers.">

![avoid unaccepted container shapes](images/16_Containers_6_dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont caption="Always optically center-align pictograms in their containers.">

![optically center align pictograms in containers](images/16_Containers_7_do.svg)

</DoDont>
<DoDont type="dont" caption="Don’t crop pictograms in container.">

![never crop pictograms in container](images/16_Containers_8_dont.svg)

</DoDont>
</DoDontRow>

## Clearance

When designing with pictograms, all artwork should include minimum padding based on 1/4 of the scaled grid size. 
The padding can be increased by increments of 1/4 grid units.

<ArtDirection>

![padding applies regardless of container shape](images/usage_v2/19-idl-iconography-pictograms-usage.png)

![padding applies regardless of container shape](images/usage_v2/19-idl-iconography-pictograms-usage.png)

</ArtDirection>

<DoDontRow>
<DoDont caption="Follow the clearance rule to allow for legibility and touch.">

![allow clearance for legibility and touch](images/usage_v2/20-idl-iconography-pictograms-usage.png)

</DoDont>
<DoDont type="dont" caption="Don’t collapse the pictogram clearance area.">

![keep pictogram clearance area](images/usage_v2/21-idl-iconography-pictograms-usage.png)

</DoDont>
</DoDontRow>

## Color

Pictograms on backgrounds must always pass color contrast requirements. 
When pairing pictograms with backgrounds, follow color-family rules to ensure that the pictogram does not clash with or blend into the background. 
Dark background colors should range between values 70–100 while light backgrounds should not exceed values 10–20.

<ArtDirection>

![pictogram with color contrast example](/images/10_Backgrounds-mobile_1.svg)
![pictogram with color contrast example](/images/10_Backgrounds_1.svg)

</ArtDirection>

<DoDontRow>
<DoDont caption="Follow the 5-step color rule and only match tones from the same color family or use grayscale backgrounds.">

![pictogram background example](/images/10_Backgrounds_2_do.svg)

</DoDont>
<DoDont caption="Follow gradient rules when placing them on backgrounds.">

![pictogram background example](/images/10_Backgrounds_3_do.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont type="dont" caption="Don’t place dark tones on dark backgrounds.">

![pictogram background example](/images/10_Backgrounds_4_dont.svg)

</DoDont>
<DoDont type="dont" caption="Don’t place light tones on light backgrounds.">

![pictogram background example](/images/10_Backgrounds_5_dont.svg)

</DoDont>
</DoDontRow>

<DoDontRow>
<DoDont type="dont" caption="Don’t place gradient picto’s on backgrounds that are not 80 and above or 20 and below.">

![pictogram background example](/images/10_Backgrounds_6_dont.svg)

</DoDont>
<DoDont type="dont" caption="Don’t place gradient picto’s on gradient backgrounds.">

![pictogram background example](/images/10_Backgrounds_7_dont.svg)

</DoDont>
</DoDontRow>

### Expressive pictogram color

Expressive pictograms have four color themes: dark, light, monochromatic dark, and monochromatic light. 
To ensure accessibility, use the color theme that corresponds with the pictogram’s background color.

<ArtDirection>

![Expressive pictogram in 4 color themes](images/usage_v2/29-idl-iconography-pictograms-usage.png)
![Expressive pictogram icon in 4 color themes](images/usage_v2/29-idl-iconography-pictograms-usage.png)

</ArtDirection>

| Background color value | Pictogram theme             |
| ---------------------- | --------------------------- |
| White, 10–20           | Light                       |
| 30–50                  | Monochromatic light (Black) |
| 50–70                  | Monochromatic dark (White)  |
| 80–100, Black          | Dark                        |

<DoDontRow>
<DoDont caption="Use dark-theme pictograms on dark backgrounds and light-theme pictograms on light backgrounds.">

![Expressive pictogram with correct color theme](images/expressivePictograms_usage-colorDo_v2.svg)

</DoDont>
<DoDont type="dont" caption="Don’t use light-theme pictograms on dark backgrounds or dark-theme pictograms on light backgrounds.">

![Expressive pictogram with incorrect color theme](images/expressivePictograms_usage-colorDont_v2.svg)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont type="dont" caption="Don’t place expressive pictograms on backgrounds between 30 and 70. Use the monochromatic version for this case.">

![Expressive pictogram on Blue 60](images/expressivePictograms_usage-colorDont_3.svg)

</DoDont>
<DoDont type="dont" caption="Don’t place expressive pictograms on gradient backgrounds.">

![Expressive pictogram on gradient background](images/expressivePictograms_usage-colorDont_4_v2.svg)

</DoDont>
</DoDontRow>

## Pictograms as illustrations

Pictograms can serve as a base for more complex illustrations, helping speed up design and ensure consistency. 
They can also be scaled for workspace environments, like wall art, bringing the brand into the environment in a bold, cohesive way.

<ImageGallery>

<ImageGalleryImage alt="pictogram on sign" title="Pictogram on sign" col={8}>

![](images/usage_v2/34-idl-iconography-pictograms-usage-v2.png)

</ImageGalleryImage>

<ImageGalleryImage alt="pictogram on sign" title="Pictogram on sign" col={4}>

![](images/usage_v2/35-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

</ImageGallery>

## Pictograms in action

<ImageGallery>

<ImageGalleryImage alt="" title="Pictogram on sign" col={12}>

![](images/usage_v2/36-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Expressive pictogram on web" col={4}>

![](images/usage_v2/37-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Pictogram on hat" col={8}>

![](images/usage_v2/38-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Expressive pictogram on poster" col={8}>

![](images/usage_v2/39-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Pictogram on card" col={4}>

![](images/usage_v2/40-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Pictogram on wallpaper" col={12}>

![](images/usage_v2/41-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Pictogram on wallpaper" col={4}>

![](images/usage_v2/42-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

<ImageGalleryImage alt="" title="Pictogram on wallpaper" col={8}>

![](images/usage_v2/43-idl-iconography-pictograms-usage.png)

</ImageGalleryImage>

</ImageGallery>
