...
IBM pictograms are offered in two different types: productive and expressive. The standard productive pictograms feature simple line-work and are the go-to pictogram type for most contexts, while the more dynamic expressive pictograms work best for select use cases when more visual impact is needed. Both types share much of the same design guidance with the exception of stroke specifications and color application.
...
Expressive strokes differ from productive strokes by stroke weight and use of gradient. When making a productive pictogram into an expressive pictogram, productive strokes remain at 0.72px while expressive strokes increase 2x to 1.44px.
...
<DoDont caption="Do use a 45º angle when applying gradients.">
...
Expressive pictograms have two elements where color is applied: the color gradient and the productive strokes. To ensure sufficient contrast for both elements, expressive pictograms are created in themes for both light and dark backgrounds. The color gradient follows the same guidance for [gradients for productive pictograms](#gradients), while the productive strokes are limited to white for dark themes and black for light themes.
...
The color gradient provides color to the gradient strokes of the pictogram. This layer remains stationary on the icon grid at 45° and is masked by the gradient strokes for consistent color across each pictogram.
