---
title: 8-Bar
description: The IBM logo is one of our most valuable corporate assets and is among the most recognized identities in the world. It's the tangible symbol of our brand.
---

<PageDescription>

The IBM logo is one of our most valuable corporate assets and is among the
most recognized corporate identities in the world, uniquely distinguishing us
from our competitors and other companies. It’s the tangible symbol of our
brand, representing everything we are: our expertise, our values, our people,
our offerings.

</PageDescription>

<AnchorLinks>
  <AnchorLink>The 8-Bar</AnchorLink>
  <AnchorLink>Negative 8-Bar</AnchorLink>
  <AnchorLink>Positive and negative</AnchorLink>
  <AnchorLink>Legal requirements</AnchorLink>
  <AnchorLink>Color</AnchorLink>
  <AnchorLink>Usage</AnchorLink>
  <AnchorLink>Lineage</AnchorLink>
  <AnchorLink>Specific use logos</AnchorLink>
  <AnchorLink>Third party logo usage</AnchorLink>
  <AnchorLink>Get in touch</AnchorLink>
</AnchorLinks>

## Resources

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="IBM Logo Request Tool in Brand Center"
      href="https://prdpcrhibmbl01.w3-969.ibm.com/marketing/logotool/logotool.nsf/BrandingLogoHome?Openform"
      >

![](/images/ibm.svg)

  </ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="The IBM business partner marks"
      href="https://www.ibm.com/brand/logos/business-partners/"
      >

![](/images/bee.svg)

  </ResourceCard>
</Column>
</Row>

## The 8-Bar

Created by legendary designer Paul Rand, the basic design of the IBM logo has remained unchanged since 1972. Its consistent, visible use reinforces the IBM brand, makes it more memorable and authenticates the things to which it is applied. It is an essential component of the IBM Look—used to lend authority and engender trust wherever it appears. It’s our responsibility to protect it.

_Note: The black stripes have been drawn thicker than white stripes. The black and white should appear to be similar optically._

![8 bar](/images/8bar.svg)

## Negative 8-Bar

The logotype has been adjusted to work well reversed on dark backgrounds. In this case the white stripes are thinner than the black stripes to adjust for optical differences but like the positive version the stripes should appear similar in weight.

![negative 8 bar](/images/8bar-neg.svg)

## Positive and negative

In addition to the differences in bar thickness between the positive and negative versions there has been adjustments to the points in the counter shape. The positive being a sharp and the negative more blunt. This subtle difference between the two ensure optical integrity on light or dark backgrounds.

<Row>
<Column colMd={4} colLg={4}>

![positive /negative 8 bar](/images/pos-neg-1.svg)

</Column>
<Column colMd={4} colLg={4}>

![positive /negative 8 bar](/images/pos-neg-2.svg)

</Column>
</Row>

## Legal requirements

The IBM logo should appear on every piece of official IBM communication. On printed literature, it generally appears on the front and/or back covers. In video, it generally appears at the end as a final sign off. In the United States, one appearance of the IBM logo (generally the signoff) must be accompanied by the ® mark. In other countries, local legal counsel should be consulted for trademark guidance. There is also a copyright statement that should appear in the legal attribution area, which reads: © International Business Machines [insert current year].

<Row>
<Column colLg={8}>
<ArtDirection>

![8 bar legal](/images/8bar-legal-mobile.svg)
![8 bar legal](/images/8bar-legal.svg)

</ArtDirection>
</Column>
</Row>

## Color

The IBM core colors consisting of the blue and gray families are used when applying color to the 8-Bar. Always use a minimum of five “steps” away between the foreground and background color to insure appropriate contrast and legibilty.

Dark or light background colors work well with the core color IBM logo. Always use a minimum of five “steps” away between the foreground and background color to insure appropriate contrast and legibilty as in the samples below.

<Row>
<Column colMd={4} colLg={4}>

![core black](/images/core-black.svg)

<Caption>Black</Caption>

</Column>
<Column colMd={4} colLg={4}>

![core blue 100](/images/core-blue100.svg)

<Caption>Blue 100</Caption>

</Column>
<Column colMd={4} colLg={4}>

![core blue80](/images/core-blue80.svg)

<Caption>Blue 80</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![core blue 70](/images/core-blue70.svg)

<Caption>Blue 70</Caption>

</Column>
<Column colMd={4} colLg={4}>

![gray 70](/images/core-gray70.svg)

<Caption>Gray 70</Caption>

</Column>
<Column colMd={4} colLg={4}>

![gray 60](/images/core-gray60.svg)

<Caption>Gray 60</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![blue 50](/images/core-blue50-black.svg)

<Caption>Blue 50 / Black</Caption>

</Column>
<Column colMd={4} colLg={4}>

![blue 40 / blue 90](/images/core-blue40-blue90.svg)

<Caption>Blue 40 / Blue 90</Caption>

</Column>
<Column colMd={4} colLg={4}>

![blue 50 / blue 100](/images/core-blue50-blue100.svg)

<Caption>Blue 50 / Blue 100</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![Gray 40 / Gray 100](/images/core-gray40-gray100.svg)

<Caption>Gray 40 / Gray 100</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Gray 30 / Blue 80](/images/core-gray30-blue80.svg)

<Caption>Gray 30 / Blue 80</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Black / Cool Gray 30](/images/core-black-coolgray30.svg)

<Caption>Black / Cool Gray 30</Caption>

</Column>
</Row>

### Backgrounds for core logos

Dark or light background colors work well with the core color IBM logo. Always use a minimum of five “steps” away between the foreground and background color to insure appropriate contrast and legibilty as in the samples below.

<Row>
<Column colMd={4} colLg={4}>

![Gray 40 / Purple 90](/images/core-gray40-purple90.svg)

<Caption>Gray 40 / Purple 90</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Gray 40 / Teal 90](/images/core-gray40-teal90.svg)

<Caption>Gray 40 / Teal 90</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Gray 30 / Cyan 80](/images/core-gray30-cyan80.svg)

<Caption>Gray 30 / Cyan 80</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![Blue 40 / Purple 90](/images/core-blue40-purple90.svg)

<Caption>Blue 40 / Purple 90</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Blue 40 / Teal 90](/images/core-blue40-teal90.svg)

<Caption>Blue 40 / Teal 90</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Blue 40 / Cyan 80](/images/core-blue40-cyan80.svg)

<Caption>Blue 40 / Cyan 80</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![Blue 40 / Blue 100](/images/core-blue40-blue100.svg)

<Caption>Blue 40 / Blue 100</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Gray 30 / Purple 80](/images/core-gray30-purple80.svg)

<Caption>Gray 30 / Purple 80</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Gray 30 / Red 90](/images/core-gray30-red90.svg)

<Caption>Gray 30 / Red 90</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![Gray 100 / blue 20](/images/core-gray100-blue20.svg)

<Caption>Gray 100 / blue 20</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Blue 90 / Purple 20](/images/core-blue90-purple20.svg)

<Caption>Blue 90 / Purple 20</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Black / Purple 20](/images/core-black-purple20.svg)

<Caption>Black / Purple 20</Caption>

</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>

![Gray 100 / Blue 10](/images/core-gray100-blue10.svg)

<Caption>Gray 100 / Blue 10</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Blue 90 / Magenta 10](/images/core-blue90-magenta10.svg)

<Caption>Blue 90 / Magenta 10</Caption>

</Column>
<Column colMd={4} colLg={4}>

![Blue 80 / Purple 10](/images/core-blue80-purple10.svg)

<Caption>Blue 80 / Purple 10</Caption>

</Column>
</Row>

### Backgrounds for white logo

There are many color backgrounds available to choose from when using the white logo. In order to achieve clear legibilty choose from the Black–60 range from the [IBM color palette](/color).

![logo backgrounds palette](/images/logos-backgrounds-palette.svg)

### Colors to avoid

Do not use colors for our logo that are not the IBM “core colors” and also please do not use unnaproved color combos that have been noted in the [IBM color palette guidance](https://www.ibm.com/color).

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont">

![example of colors not to use](images/usage-donot-1.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont">

![example of colors not to use](images/usage-donot-2.svg)

</DoDontExample>
</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont">

![example of colors not to use](images/usage-donot-3.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont">

![example of colors not to use](images/usage-donot-4.svg)

</DoDontExample>
</Column>
</Row>

## Usage

The following examples demonstrate important points to consider when using the logo across a wide range of applications. The logo should not be altered in any way and should always adhere to following guidance.

### Clear space

The IBM logo should always be clearly visible. We keep the logo separated from other visual elements by a distance equal to its height. This distance is considered the minimum uninterrupted space surrounding the logo. We call it the area of isolation clear space and should be adhered to in most situations.

![log clearspace](images/logo-clearspace.png)

### Alignments

The IBM logo has both horizontal and vertical relationships with the business units and hero brand logotypes. In either axis the IBM logo is based on the cap height of the logotype or can scale larger by the ration oulined in the next section below. Please make sure to use the appropriate positve or negative version of the logo.

![logo alignments](images/logo-alignments.svg)

### Scale relationships

The IBM logo has several size relationships with logotypes. These relationships are driven by the cap-height of the logotypes and insure a nice balance with the IBM logo. The logo can “hang” from the cap-height or align to the “baseline” when the logos are placed near the bottom edge of a layout or footer.

![logo scale](images/logos-scale.svg)

### Usage on photography

The Logotype should be carefully placed on photography. Always place on clear and simple backgrounds with plenty of contrast for legibilty. Avoid placing on busy or complex backgrounds or images to keep the 8-Bar as legible as possible.

<Row>
<Column colMd={4} colLg={6}>
<DoDontExample type="do">

![logo on photography example](images/photography-do-1.png)

</DoDontExample>
</Column>
<Column colMd={4} colLg={6}>
<DoDontExample type="do">

![logo on photography example](images/photography-do-2.png)

</DoDontExample>
</Column>
</Row>

<Row>
<Column colMd={4} colLg={6}>
<DoDontExample type="dont">

![logo on photography example](images/photography-dont-1.png)

</DoDontExample>
</Column>
<Column colMd={4} colLg={6}>
<DoDontExample type="dont">

![logo on photography example](images/photography-dont-2.png)

</DoDontExample>
</Column>
</Row>

### Things to avoid

The logotype should not be placed into container shapes, altered or added to in any way. It should only be used in the appropriate colors from the IBM color palette. Below reflects only a handful of samples of what not to do.

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not change stripes direction/weight">

![ dont change stripe direction example](images/usage-dont-stripe-direction.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not make a dot matrix">

![ dot matrix example](images/usage-dont-dotmatrix.svg)

</DoDontExample>
</Column>
</Row>
<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not extend serifs to touch">

![ don't extend serifs example](images/usage-dont-serifs-touch.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not outline">

![dont outline example](images/usage-dont-outline.svg)

</DoDontExample>
</Column>
</Row>
<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not stretch or compress">

![dont stretch example](images/usage-dont-stretch.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not alter spacing">

![don't alter spacing example](images/usage-dont-spacing.svg)

</DoDontExample>
</Column>
</Row>
<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not outline and overlap">

![don't outline and overlap example](images/usage-dont-overlap.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not place in container shape">

![don't place in container shape example](images/usage-dont-container.svg)

</DoDontExample>
</Column>
</Row>
<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not add the “thought rays”">

![don't add thought rays example](images/usage-dont-thoughtrays.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not add characters in same style">

![don't add characters in same style example](images/usage-dont-add.svg)

</DoDontExample>
</Column>
</Row>

## Lineage

The IBM logo, along with the business it signified, transformed through its initial decades. In 1924 the name changed to International Business Machines and thus began the evolution of IBM’s brand identity. The gallery below illustrates its journey.

![logo lineage](images/logo-lineage.png)

## Specific use logos

There are two additional logo styles that can be used in special applications such as merchandise, badges and events. These logos should not be used in place of the 8-Bar as an official brand communications sign-off.

### Solid logo

The solid logo is the predecessor to the 8-Bar and is optimized for usage at smaller scales based on its simplified character. Works well in digital and print applications where the 8-Bar doesn’t render well at small sizes. The negative version is lighter in weight to compensate for optical differences.

![solid 8 bar logo](images/8bar-solid-lg.svg)

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="do" caption="The solid logo works well in headers and footers when space is minimal.">

![solid logo in header](images/8bar-solid-header-do.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not use the solid logo for medium to large scale applications.">

![solid logo in large application example](images/8bar-solid-large-applications-dont.svg)

</DoDontExample>
</Column>
</Row>
<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="do" caption="The solid logo should be used when including ‘IBM’ in app icons.">

![solid logo example](images/8bar-solid-app-icons-do.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not use the solid logo in place of the 8-Bar for partnerships.">

![solid logo example with parternship](images/8bar-solid-partnerships-dont.svg)

</DoDontExample>
</Column>
</Row>

## Diversity 8-Bar

At IBM, we have for over 100 years celebrated diversity and championed equal rights for every individual. For the gay rights movement 50 year anniversary, we payed tribute to Baker’s original vision adapting our corporate logo to demonstrate our solidarity, support and continued commitment to the rights of the LGBT community.

![diversity 8-bar](images/logo-lgbt.svg)

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="do" caption="Use the Diversity logo as an identifier on physical badges and social avatars.">

![diversity 8-bar example](images/8bar-LGBT-social-do.svg)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not use in place of the 8-Bar as a marketing sign off.">

![diversity 8-bar bad example](images/8bar-LGBT-signoff-dont.svg)

</DoDontExample>
</Column>
</Row>

<Row>
<Column colMd={4} colLg={4}>
<DoDontExample type="do" caption="The Diversity logo can be applied to merchandise.">

![diversity merchandise example](images/8bar-LGBT-merchandise-do.png)

</DoDontExample>
</Column>
<Column colMd={4} colLg={4}>
<DoDontExample type="dont" caption="Do not apply to background colors other than White or Gray 10.">

![diversity merchandise bad example](images/8bar-LGBT-bgcolor-dont.svg)

</DoDontExample>
</Column>
</Row>

## Third party logo usage

There are multiple categories of IBM logo use requests we consider to be third party. These may come from partners that want to market their solutions as enabled by IBM, or associations and events IBM has sponsored around the world, or co-marketing requests. Each is evaluated individually, with the key criteria being appropriateness, clarity of relationship, clarity of ownership of communications or offering, and general liability etc. IBM requires all divisions to comply with these guidelines to ensure a consistent IBM brand identity across all messaging.

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="IBM Logo Request Tool in Brand Center"
      href="https://prdpcrhibmbl01.w3-969.ibm.com/marketing/logotool/logotool.nsf/BrandingLogoHome?Openform"
      >

![](/images/ibm.svg)

  </ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
    <ResourceCard
      subTitle="The IBM business partner marks"
      href="https://www.ibm.com/innovate/brand/ibm-logo/"
      >

![](/images/bee.svg)

  </ResourceCard>
</Column>
</Row>

## Get in touch

<Row>
<Column colMd={4} colLg={4}>

**Huda Mohamed**<br/>
Brand Strategist, Brand Experience & Design<br/>
[mohamed@ca.ibm.com](mailto:mohamed@ca.ibm.com)

</Column>
<Column colMd={4} colLg={4}>

**Lisa Lind**<br/>
Executive Producer, Brand Experience & Design<br/>
[llind@us.ibm.com](mailto:llind@us.ibm.com)

</Column>
</Row>
