---
title: Technical diagrams
description: "Build technical diagrams with clarity based on the IBM Design Language and Carbon. From depicting IT architecture to product documentation, use this guidance."
keywoords: "IBM technical diagrams, technical architecture diagrams, technical process diagrams"
tabs: [Design, Usage]
---

<PageDescription>

Technical diagrams require thoughtful arrangement and application of a wide variety of elements to deliver information with clarity. Consider the following guidelines when creating technical diagrams. 


</PageDescription>

<AnchorLinks>
  <AnchorLink>Examples in use</AnchorLink>
  <AnchorLink>Clarity</AnchorLink>
  <AnchorLink>Carbon</AnchorLink>
  <AnchorLink>Subsystems</AnchorLink>
</AnchorLinks>

## Examples in use

<ImageGallery>

<ImageGalleryImage alt="Technical diagram on a web page" title="Technical diagram on a web page" col={4}>

![Technical diagram on a web page](./images/usage/examples/example1.png)

</ImageGalleryImage>
<ImageGalleryImage alt="Poster printout of technical diagram to encourage team input" title="Poster printout of technical diagram to encourage team input" col={8}>

![Poster printout of technical diagram to encourage team input](./images/usage/examples/example2.jpg)

</ImageGalleryImage>
<ImageGalleryImage alt="technical diagram with dark theme background on laptop slide presentation" title="technical diagram with dark theme background on laptop slide presentation" col={8}>

![technical diagram with dark theme background on laptop slide presentation](./images/usage/examples/example3.png)

</ImageGalleryImage>
<ImageGalleryImage alt="technical diagram within a white paper" title="technical diagram within a white paper" col={4}>

![technical diagram within a white paper](./images/usage/examples/example4.png)

</ImageGalleryImage>

</ImageGallery>

## Clarity

When creating a technical diagram, clarity and legibility are paramount. These guidelines were developed to help, as well as ensure brand consistency. Try to follow them as closely as possible. Depending on the tools used to create diagrams, there may be certain limitations, however clarity should never be sacrificed.

<DoDontRow>
<DoDont type="do" aspectRatio="1:1" caption="Always add a legend to your diagram, showing the color, line, style and shape conventions used.">

![Always add a legend to your diagram, showing the color, line, style and shape conventions used.](./images/usage/clarity/dodont1.png)

</DoDont>
<DoDont type="dont" aspectRatio="1:1" caption="Don’t use icons by themselves to represent essential information—always use words, as well.">

![Don’t use icons by themselves to represent essential information—always use words, as well.](./images/usage/clarity/dodont2.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont type="do" aspectRatio="1:1" caption="When a diagram can’t be displayed at 100% scale, offer the user a way to download it or zoom in.">

![When a diagram can’t be displayed at 100% scale, offer the user a way to download it or zoom in.](./images/usage/clarity/dodont3.png)

</DoDont>
<DoDont type="do" aspectRatio="1:1" caption="Alternate background fill colors to improve nesting readability.">

![Alternate background fill colors to improve nesting readability.](./images/usage/clarity/dodont4.png)

</DoDont>
</DoDontRow>
<DoDontRow>
<DoDont type="dont" aspectRatio="1:1" caption="Avoid redundancy when using shapes, styles, colors or indicator badges.">

![Avoid redundancy when using shapes, styles, colors or indicator badges.](./images/usage/clarity/dodont5.png)

</DoDont>
<DoDont type="dont" aspectRatio="1:1" caption="Don’t mix types of nodes unless they have specific semantic differences that can’t be expressed otherwise.">

![Don’t mix types of nodes unless they have specific semantic differences that can’t be expressed otherwise.](./images/usage/clarity/dodont6.png)

</DoDont>
</DoDontRow>

## Carbon

The technical diagrams guidance presented here is meant for diagrams that appear outside of product screens, such as those found in documentation websites, in presentations, on collateral and so on. When creating a technical diagram as part of a product, use the the Carbon components and guidance instead. 

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Carbon network diagrams guidance"
  href="https://www.carbondesignsystem.com/data-visualization/complex-charts/#network-diagrams"
  actionIcon="launch"
>

![Carbon](../../../images/resource-cards/carbon.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Carbon network diagrams storybook"
  href="https://carbon-design-system.github.io/carbon-charts/angular/?path=/story/diagrams--start-here"
  actionIcon="launch"
>

![Storybook](../../../images/resource-cards/storybook.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Carbon tree diagrams guidance"
  href="https://www.carbondesignsystem.com/data-visualization/complex-charts/#tree-diagrams"
  actionIcon="launch"
>

![Carbon](../../../images/resource-cards/carbon.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Carbon tree diagrams storybook"
  href="https://carbon-design-system.github.io/carbon-charts/?path=/story/complex-charts-tree--dendrogram"
  actionIcon="launch"
>

![Storybook](../../../images/resource-cards/storybook.png)

</ResourceCard>
</Column>
</Row>

## Subsystems

Using our technical diagram guidance as a foundation, some teams have expanded on our system. They’ve built a number of discrete extensions, effectively creating subsystems that specifically address their exact use cases. If you work with one of these subsystems, do follow our general guidance, as well as the specific adaptations applicable to your use case. If you believe you have an application that requires new elements, reach out to the IBM brand design team with your proposal for approval. It helps ensure that your technical diagram subsystem adheres to our general guidance, and gives us the opportunity to share it here with all teams.

### IT architecture

When creating an IT architecture diagram, always reference the [IBM Systems Description Standard (SDS)](https://w3.ibm.com/w3publisher/it-architect-key-techniques-assets/ibm-systems-description-standard-sds) and follow the visual guidance developed specifically for this type of diagram. Doing so helps ensure correct interpretation of your architectural model. 

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="IBM Systems Description Standard (SDS)"
  href="https://w3.ibm.com/w3publisher/it-architect-key-techniques-assets/ibm-systems-description-standard-sds"
  actionIcon="launch"
>

![Carbon](../../../images/resource-cards/carbon.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Cognitive Architect"
  href="http://ibm.biz/cogarch-app"
  actionIcon="launch"
>

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Diagrams.net library"
  href=" https://github.com/ibm-cloud-architecture/ibm-cloud-stencils/releases"
  actionIcon="launch"
>

![Draw.io](../../../images/resource-cards/drawio.png)

</ResourceCard>
</Column>
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Illustrator kit"
  href=""
  actionIcon="launch"
>

![Adobe Illustrator](../../../images/resource-cards/illustrator.png)

</ResourceCard>
</Column>
</Row>

### Product documentation

All products created by IBM have some related documentation. Often, this documentation includes various types of technical diagrams, such as flows, process diagrams, network diagrams and so on. Whenever you need to create a diagram representing a system of parts and the relationship between them, start from our general technical diagrams guidance. Various teams have developed subsystems tailored to their specific applications. Check this Box folder for customized kits.

<Row className="resource-card-group">
<Column colMd={4} colLg={4} noGutterSm>
<ResourceCard
  subTitle="Various kits for documentation diagrams"
  href=""
  actionIcon="launch"
>

![Box](../../../images/resource-cards/box.png)

</ResourceCard>
</Column>
</Row>