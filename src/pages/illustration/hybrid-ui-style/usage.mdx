...
When creating a hybrid UI illustration for an AEM leadspace, we recommend using the Leadspace with foreground asset component. For best legibility, draw the artboard twice the size of the needed image on the large breakpoint. 

If you need to use the Leadspace with background asset component, make sure to leave enough clear space around the image so that illustrations don’t overlap with the type or buttons at any breakpoint. 
